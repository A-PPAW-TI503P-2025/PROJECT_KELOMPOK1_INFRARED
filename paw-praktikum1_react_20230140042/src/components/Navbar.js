...
import { Menu, X, LogOut, Activity } from "lucide-react"; // Import ikon Activity
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center">
            <Link to="/dashboard" className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center shadow-md">
                <Activity className="text-white h-6 w-6" /> {/* Ganti teks AP dengan ikon */}
              </div>
          ...
          <div className="hidden md:flex items-center space-x-3">
            ...
            {/* Menu Khusus Admin: Data Pasien */}
            {user?.role === "admin" && (
              <Link
                to="/pasien"
                className={`px-5 py-2.5 rounded-full font-medium transition-all ${
                  isActive("/pasien") ? activeClass : inactiveClass
                }`}
              >
                Data Pasien
              </Link>
            ...
            {/* Menu Monitoring Suhu (Bisa diakses semua role) */}
            <Link
              to="/monitoring"
              className={`px-5 py-2.5 rounded-full font-medium transition-all ${
          ...
          <div className="px-4 py-5 space-y-3">
            ...
            {user?.role === "admin" && (
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={`block px-6 py-3 rounded-full font-medium ${
                  isActive("/pasien") ? activeClass : "text-gray-700"
                }`}
              >
                Data Pasien
              </Link>
