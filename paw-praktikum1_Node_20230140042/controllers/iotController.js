...
exports.receiveSensorData = async (req, res) => {
  try {
    ...
    const { suhu, kelembaban, cahaya, no_kamar } = req.body;
    ...
    const newData = await SensorLog.create({
      ...
      no_kamar: no_kamar || "General"    
    });

    console.log(`[IOT] Kamar ${newData.no_kamar} | Suhu: ${suhu}°C`);
    res.status(201).json({ status: "ok", message: "Data tersimpan" });
...
exports.getSensorHistory = async (req, res) => {
  try {
    ...
    let whereClause = {};
    if (kamar) whereClause.no_kamar = kamar;
...
exports.testConnection = (req, res) => res.json({status: "ok"});
